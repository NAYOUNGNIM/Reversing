...
def keygen17(name):
    ...
    e_edi='%x' %e_edi
    e_ebx='%x' %e_ebx
    print e_esi[:4].upper(), '-', mem1[:4].upper(), '-', e_edi[:4].upper(), '-', e_ebx[:4].upper()

#출처: http://bbolmin.tistory.com/133
...
def keygen18(name):
    ...
    for i in range(0x10):
        ...
        esi=esi>>0x10
        esi&=0xff
        tmp=mem408db8[edi*4:edi*4+4]
        tmp=tmp[::-1]
        ebx=int(''.join(tmp), 16)
        tmp=mem4091b8[esi*4:esi*4+4]
        tmp=tmp[::-1]
        tmp=''.join(tmp)
        tmp=int(tmp, 16)
        ebx=(ebx+tmp)&0xffffffff
        edi=(eax&0x0000ff00)>>8
        esi=eax&0x000000ff
        tmp=mem4095b8[edi*4:edi*4+4]
        tmp=tmp[::-1]
        ebx^=int(''.join(tmp), 16)
        tmp=mem4099b8[esi*4:esi*4+4]
        tmp=tmp[::-1]
        tmp=''.join(tmp)
        tmp=int(tmp, 16)
        ebx=(ebx+tmp)&0xffffffff
    ...
    eax=int(eax[2:4], 16)
    edx&=0xffffff00
    edx|=int(mem404768[l*2], 16)
    eax=(edx+eax)&0xffffffff
    eax=(eax*0x407af0)&0xffffffff
    edx=(edx+eax)&0xffffffff
    eax=(eax-0x587370)&0xffffffff
    s_eax=eax
    ebx=eax
    eax=0
    ebx=(ebx*0x4076f0)&0xffffffff
    eax&=0x4076f0
    edx=hex(edx)[2:]
    eax=hex(s_eax)[2:]
    eax='0'*(8-len(eax))+eax
